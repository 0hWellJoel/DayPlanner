...
function initPage() {
  var tableRow = document.querySelectorAll(".eachhour");
  var strText = "";
  var nIndex = 0;
  var nHour = 0;
  ...
  for (var i = 0; i < tableRow.length; i++) {
    ...
    strText += $("#hour") + nIndex + "-" + nHour + " ";

    if (nHour < d) {
      $("#hour" + nIndex + "-" + nHour).addClass("past");
      $("#textbox" + nIndex).attr("disabled", "disabled"); //past rows are disabled
      $("#save" + nIndex).addClass("disabled", true);
    } else if (nHour == d) {
      $("#hour" + nIndex + "-" + nHour).addClass("present"); //present and future are editable
    } else {
      $("#hour" + nIndex + "-" + nHour).addClass("future");
    }
  }
}
